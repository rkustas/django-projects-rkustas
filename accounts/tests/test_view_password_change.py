...
class LoginRequiredPasswordChangeTests(TestCase):
    def test_redirection(self):
        ...
        # Try to access password_change withou being logged in, redirects to login
...
class SuccessfulPasswordChangeTests(PasswordChangeTestCase):
    ...
    def test_redirection(self):
        # Valid form submission redirects user
    ...
    def test_user_authentication(self):
        # Create new request to arbitrary page.  The response should now have a 'user' to its context after successful signup
        response = self.client.get(reverse('home'))
        user = response.context.get('user')
        self.assertTrue(user.is_authenticated)


class InvalidPasswordChangeTests(PasswordChangeForm):
    def test_status_code(self):
        # Invalid form submission returns same page
        self.assertEquals(self.response.status_code, 200)
    ...
    def test_didnt_change_password(self):
        # refresh user instance from db to make sure we have latest data
        self.user.refresh_from_db()
